...
#define ASK_TABLE_SIZE 80   // 每bit采样点数 = 80 (40ms @ 2kHz触发, 25bps)
...
// 80点25Hz正弦载波表 (bit=1: 完整周期, DC平衡, 幅度931, 能量~47200)
static const u16 g_ask_one[ASK_TABLE_SIZE] = {
    2048, 2121, 2194, 2265, 2336, 2404, 2471, 2534, 2595, 2653,
    2706, 2756, 2801, 2842, 2878, 2908, 2934, 2953, 2968, 2976,
    2979, 2976, 2968, 2953, 2934, 2908, 2878, 2842, 2801, 2756,
    2706, 2653, 2595, 2534, 2471, 2404, 2336, 2265, 2194, 2121,
    2048, 1975, 1902, 1831, 1760, 1692, 1625, 1562, 1501, 1443,
    1390, 1340, 1295, 1254, 1218, 1188, 1162, 1143, 1128, 1120,
    1117, 1120, 1128, 1143, 1162, 1188, 1218, 1254, 1295, 1340,
    1390, 1443, 1501, 1562, 1625, 1692, 1760, 1831, 1902, 1975
};
...
static u16 g_total_bits;  // 总bit数 (帧bit + 静默bit)
static u16 g_bit_index;   // 当前bit位置
static u8 g_state;        // 发送状态
static u8 g_repeat_cnt;   // 单次发送剩余重复次数(0=不重复)
...
void DAC_2ASK_Init(void)
{
    ...
    // ===== 4. TIM4: 25Hz → 40ms bit周期 (25bps,每bit含80个采样点) =====
    //     72MHz / 288 / 10000 = 25Hz
...
void DAC_2ASK_SendData(u8 *data, u8 len)
{
    ...
    g_total_bits = g_frame_len * 10 + 10; // +10bit静默期(确保RX字节同步)
    g_bit_index  = 0;
    g_repeat_cnt = 2;  // 重复发送2次
    g_state      = STATE_SENDING;
}

/* 无校验快速发送: 0xAA头+长度+数据, 无校验字节 (4字节帧) */
void DAC_2ASK_SendDataNoCS(u8 *data, u8 len)
{
    u8 i;
    if (len > MAX_DATA_LEN) len = MAX_DATA_LEN;
    ...
    g_frame_len = 2 + len;  /* 无校验 */
    g_total_bits = g_frame_len * 10 + 10;
    g_bit_index  = 0;
    g_repeat_cnt = 3;
    g_state      = STATE_SENDING;
}

/* 快速帧: 0xAA头+长度+数据, 无校验 (BCD检查替代) */
void DAC_2ASK_SendShort(u8 *data, u8 len)
{
    DAC_2ASK_SendDataNoCS(data, len);
}
