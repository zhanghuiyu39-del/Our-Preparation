...
#define ASK_TABLE_SIZE 40   // 每bit采样点数 = 40 (200ms @ 200Hz触发达)/
...
// ============================================================
// 2ASK 模式 — PA4输出20Hz载波，5bps帧调制
// ============================================================

// 40点包络载波表 (bit=1: 缓升10点 + 持续20点 + 缓降10点, 1.50Vpp)
// 升/降采用升余弦包络: env = 0.5*(1-cos(π*n/10))
static const u16 g_ask_one[ASK_TABLE_SIZE] = {
    2048, 2062, 2137, 2240, 2247, 2048, 1672, 1309, 1206, 1487,
    2048, 2623, 2979, 2979, 2623, 2048, 1473, 1117, 1117, 1473,
    2048, 2623, 2979, 2979, 2623, 2048, 1473, 1117, 1117, 1473,
    2048, 2568, 2787, 2657, 2336, 2048, 1929, 1959, 2025, 2048
};

// 10点载波表 (CW模式用, 纯净20Hz正弦波 1.50Vpp)
#define CW_TABLE_SIZE 10
static const u16 g_cw_carrier[CW_TABLE_SIZE] = {
    2048, 2623, 2979, 2979, 2623, 2048, 1473, 1117, 1117, 1473
};

// 40点零值表（bit=0时无载波输出）
static const u16 g_ask_zero[ASK_TABLE_SIZE] = {0};
...
void DAC_2ASK_Init(void)
{
    ...
    // ===== 4. TIM4: 5Hz → 200ms bit周期 (5bps,每bit含4个载波周期) =====
    //     72MHz / 360 / 40000 = 5Hz
