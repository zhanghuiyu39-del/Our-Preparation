 ...
 * 6. INV_MODE_OPEN_LOOP：接好采样板并完成分级验证后进行限流开环测试。
 */
...
#define INV_USER_WORK_MODE INV_MODE_PWM_WAVEFORM /**< 开环PWM不依赖ADC有效标志；线电压ADC仅用于观测。 */
 ...
 */
#define INV_USER_DC_BUS_V          (60.0f) /**< 开环计算使用的直流母线电压，单位V；当前对应60 V限流母线。 */
#define INV_USER_LINE_RMS_V        (32.0f) /**< 目标三相线电压基波有效值，单位V RMS；不是相电压，也不是闭环保证值。 */
...
/** 1表示只处理三路线电压；当前IOC仍转换电流Rank，但软件忽略其贴轨和物理量结果。 */
