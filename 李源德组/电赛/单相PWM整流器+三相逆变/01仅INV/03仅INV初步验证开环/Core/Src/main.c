...
/*
 * 临时SVPWM功率板验证模式：
 * 1 = 不连接ADC采样板，跳过零偏、贴轨、AWD及三ADC同步保护；ADC3 DMA
 *     仅作为10 kHz控制节拍，不读取其浮空采样值。
 * 0 = 恢复完整ADC3/4/5同步采样和保护，接入采样板后必须改回0。
 *
 * 该开关不会关闭PB10/HRTIM1_FLT3、CSS、HardFault和控制心跳监督。
 * 当前只能用于5 V限流电源下的开环验证，禁止用于提高母线电压后的测试。
 */
...
#if (INV_SVPWM_ONLY_NO_ADC_MODE == 0U)
...
#else
  /*
   * 无采样板模式不读取measurement。ADC3 DMA只提供10 kHz回调节拍，
   * 因此传入NULL是预期行为，不代表软件错误。
   */
  (void)measurement;
#endif
...
int main(void)
{
  ...
  /*
   * 完整模式必须等待ADC零点和同步链路通过。临时无采样板模式跳过该步骤，
   * 否则浮空输入会触发贴轨故障，导致HRTIM输出无法开放。
   */
#if (INV_SVPWM_ONLY_NO_ADC_MODE == 0U)
  ...
  /* PB10/FLT3正常后进入READY；无采样板模式不把ADC结果作为启动条件。 */
...
#else
      /*
       * 无采样板模式只监督SVPWM控制心跳、FLT3和应用状态。ADC3 DMA若停止，
       * inv_control_heartbeat也会停止，因此仍可在100 ms内关闭PWM并停止喂狗。
       */
...
#else
  /*
   * 临时模式只使用ADC3双Rank DMA完成事件作为10 kHz控制节拍。
   * ADC4/5回调直接忽略，浮空ADC原始码不会进入测量、零偏或保护判断。
   */
  if ((hadc != NULL) && (hadc->Instance == ADC3))
  {
...
#else
  /* 临时模式不让ADC采样错误关断SVPWM；恢复采样板后必须重新启用本保护。 */
  (void)hadc;
#endif
}
...
#else
  /* 浮空采样输入不具备物理意义，临时忽略AWD；仅限5 V无采样板验证。 */
  (void)hadc;
#endif
}
