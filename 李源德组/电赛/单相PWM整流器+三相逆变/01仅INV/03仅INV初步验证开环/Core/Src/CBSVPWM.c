 ...
 * 不负责ADC采样、HRTIM寄存器更新、板级GPIO或故障状态机。
 * 数据流如下：
                                    ...
                                    uint8_t four_leg)
{
    ...
    /* 最后复核占空比，防止未来算法改动把非法值带入HRTIM比较寄存器。 */
    if ((CBSVPWM_IsFinite(svpwm->duty_u) == 0U) ||
        (CBSVPWM_IsFinite(svpwm->duty_v) == 0U) ||
        (CBSVPWM_IsFinite(svpwm->duty_w) == 0U) ||
        (svpwm->duty_u < 0.0f) || (svpwm->duty_u > 1.0f) ||
        (svpwm->duty_v < 0.0f) || (svpwm->duty_v > 1.0f) ||
        (svpwm->duty_w < 0.0f) || (svpwm->duty_w > 1.0f)) {
        CBSVPWM_SetNeutralOutput(svpwm);
        return 0U;
    }
 ...
 * @note   在控制ISR启动前调用一次；本函数不访问HRTIM或板级GPIO。
 */
