...
static uint8_t joint_inv_calibration_confirmed;
/*
...
/* RAW_ADC监督两套采样；INV-only只监督INV采样和C/D/E输出域。 */
...
static HAL_StatusTypeDef JointStartInv(const PFC_Measurement *pfc_measurement)
{
    if ((joint_pwm_enable == 0U) ||
        (joint_inv_calibration_confirmed == 0U))
    {
        return HAL_ERROR;
    }
    if ((joint_stage != PFC_INV_STAGE_INV_ONLY) &&
        ((pfc_measurement == NULL) || (pfc_measurement->valid == 0U) ||
                                  ...
                                  uint8_t iwdg_reset_seen)
{
    ...
    joint_inv_calibration_confirmed =
        (uint8_t)PFC_INV_INV_CALIBRATION_CONFIRMED;
...
void PFC_INV_AppFastInvStep(const INV_Measurement *measurement)
{
    ...
    if (!INV_OpenLoop_Step(&reference) ||
        ...
        (CBSVPWM_Calc3Leg((CBSVPWM_t *)&pfc_inv_svpwm,
                          ...
                          reference.vw_command, PFC_INV_INV_FIXED_DC_BUS_V) == 0U) ||
