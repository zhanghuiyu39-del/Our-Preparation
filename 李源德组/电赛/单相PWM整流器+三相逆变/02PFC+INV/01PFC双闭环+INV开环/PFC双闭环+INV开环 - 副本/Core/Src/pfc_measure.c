...
/*
 * pfc_measurement是ADC1 DMA ISR正在构造的工作副本，不能直接提供给外部读取。
 * DMA1_Channel1的优先级低于DMA2的逆变回调，INV ISR可能在PFC ISR尚未写完该结构体
 * 时抢占它。因此发布区采用两个缓冲区：写入者始终先完整写入“非当前”缓冲区，完成后
 * 只用一次8位索引翻转提交；读者看到的索引对应的缓冲区已经完整，不会读到半更新帧。
 */
static volatile PFC_Measurement pfc_measurement = {0};
static volatile PFC_Measurement pfc_published_measurement[2] = {{0}, {0}};
static volatile uint8_t pfc_published_index = 0U;
...
/**
 * @brief  将ADC1 ISR已经构造完成的工作副本原子地发布为最新PFC快照。
 * @note   仅由ADC1 DMA ISR或故障ISR调用。先写非活动缓冲区、执行数据屏障、最后翻转
 *         单字节索引；高优先级INV ISR即使在写入过程中抢占，也只会取得上一个完整帧。
 *         本函数不读取DMA数组、不执行浮点计算、不操作HRTIM。
 */
static void PFC_Measure_PublishSnapshot(void)
{
    uint8_t next_index = (uint8_t)(pfc_published_index ^ 1U);

    pfc_published_measurement[next_index] = pfc_measurement;
    __DMB();
    pfc_published_index = next_index;
}
...
void PFC_Measure_Init(const PFC_Params *params)
{
    ...
    pfc_published_measurement[0] = (PFC_Measurement){0};
    pfc_published_measurement[1] = (PFC_Measurement){0};
    pfc_published_index = 0U;
    pfc_params = params;
    adc2_sequence = 0U;
...
void PFC_Measure_OnAdc1Complete(void)
{
    ...
    /* 若AWD等高优先级故障ISR在本回调中途抢占，故障位已锁存，不能在末尾重新置valid。 */
    if (pfc_measurement.fault_bits != PFC_FAULT_NONE)
    {
        pfc_measurement.valid = 0U;
        PFC_Measure_PublishSnapshot();
        return;
...
{
#if PFC_USER_RELAXED_PWM_TEST != 0U
    /* 放宽模式保留AWD中断但不升级为停机；原始码和越窗次数由上层遥测观察。 */
    (void)source;
    return;
#else
    switch (source)
 ...
 * @note   临界区只保护已发布缓冲区的索引读取和结构体复制，不会冻结DMA原始数组；
 *         函数保留调用前的PRIMASK状态，既可由主循环调用，也可由高优先级INV ISR调用。
 */
void PFC_Measure_GetSnapshot(PFC_Measurement *measurement)
{
    ...
    __disable_irq();
    published_index = pfc_published_index;
    __DMB();
    *measurement = pfc_published_measurement[published_index];
