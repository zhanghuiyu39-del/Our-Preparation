 ...
 * CH0~CH6依次为VAC、IPFC、瞬时电流参考、VBUS、VBUS参考、RMS电流指令和调制量。
 * CH7在两种模式下都编码PFC状态与完整故障位，具体关系见主循环发送位置。
 */
...
int main(void)
{
  ...
  while (1)
  {
	...
	if ((HAL_GetTick() - vofa_tick) >= 10U)
	{
		 ...
		 * 其他状态：CH0~CH6发送VAC、IPFC、瞬时Iref、VBUS、VBUS参考、
		 * RMS电流指令和调制量，对应双环数据流从输入到执行器的关键节点。
		 */
		...
		else
		{
			...
			vofa_data[1] = vofa_measurement.ipfc;
			vofa_data[2] = control_telemetry.current_reference;
			vofa_data[3] = vofa_measurement.vbus;
			vofa_data[4] = control_telemetry.vbus_reference;
			vofa_data[5] = control_telemetry.current_rms_command;
			vofa_data[6] = control_telemetry.modulation;
		}

		/*
		 * CH7 = 状态*1000000 + 完整故障位。当前最大值低于float整数精确范围，
		 * 因而新增的bit16/bit17控制故障不会像旧16位打包方式那样被截断。
		 */
		vofa_data[7] = (float)((uint32_t)PFC_AppGetState() * 1000000U +
		                            vofa_measurement.fault_bits);
