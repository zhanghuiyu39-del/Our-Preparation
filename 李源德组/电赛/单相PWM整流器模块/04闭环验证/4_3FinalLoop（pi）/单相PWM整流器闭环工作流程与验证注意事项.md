...
以上内容是此前PI+PR版本的历史记录。当前`4_3FinalLoop（pi）`目录已经存在`MDK-ARM/stm32g474xx_flash.sct`，但每次修改源码后仍必须重新执行Keil Rebuild；只有本次实际构建显示`0 Error(s), 0 Warning(s)`并生成新HEX，才能进入无母线波形验收。

## 25. 2026-08-01 PI-only控制版本（当前`4_3FinalLoop（pi）`）

> 本节覆盖前文涉及PR电流环、IPFC工程量、软件过流和电流资格判断的旧说明。当前工程没有可信电流反馈，只能把IPFC当作ADC原始码观察通道。不要再用`IPFC`、`IPFC_RMS`、软件PF或旧PR变量判断当前控制是否正常。
...
```text
ADC1完整DMA回调（10 kHz唯一控制入口）
-> 读取VAC、VBUS一致性快照
-> VAC正向过零校正相位
-> VBUS每10帧进入一次100 Hz陷波器和1 kHz PI
-> PI输出虚拟输入电流I*（A RMS）
-> Ipk=sqrt(2)*I*
-> di/dt=2*pi*f*Ipk*cos(theta)
-> vL=L*di/dt
-> vbridge=VAC-vL
-> m=bridge_polarity*vbridge/VBUS
-> SPWM限幅
-> HRTIM Timer A/B Compare预装载
```

这里的`I*`和瞬时电流参考都是模型变量，不是ADC测得的输入电流。程序无法确认真实电流是否等于`I*`，也无法依靠它识别电感饱和、桥臂直通或负载短路。

`pr.c/.h`继续保留在源码目录中，便于以后修好IPFC采样后恢复电流环，但当前Keil工程不编译`pr.c`，运行路径也不调用`PR_Init()`或`PR_Calc()`。
...
S=00 SAFE
-> S=03 ADC_CHECK
-> S=04 PASSIVE_PRECHARGE
-> S=05 READY
-> S=11 PWM_PRIME
-> S=06 POWER_RAMP
-> S=07 VBUS_LOOP_RAMP
-> S=08 VBUS_LOOP_RUN
-> S=09 STOP 或 S=10 FAULT_LATCH
```

- `READY`：VAC/VBUS标定、VAC同步、母线预充和故障条件均合格，等待PD0短按。
- `PWM_PRIME`：四路输出仍关闭；程序按当前`VAC/VBUS`预装载前馈调制度，并等待启动命令之后的新正向过零。
- `POWER_RAMP`：正向过零后才开放TA1/TA2/TB1/TB2，虚拟电流从0按参数斜率增加到预置值；不再检查电流误差。
- `VBUS_LOOP_RAMP`：PI无扰接管，母线参考从接管时的实测值缓慢升到60 V。
- `VBUS_LOOP_RUN`：母线进入目标容差后维持60 V。
- `STOP`：运行中再次短按并释放PD0，程序先关闭四路PWM，再复位PI和模型状态。
- `FAULT_LATCH`：任何锁存故障后不自动恢复，必须排除原因并复位MCU。

### 25.3 日常只修改的参数

唯一人工入口为`Core/Inc/pfc_user_config.h`。
...
这一档目标为36 V RMS输入、60 V母线、100 Ω负载，输出功率约36 W；PI虚拟电流上限约1.34 A RMS，母线参考斜率约2 V/s。首次带功率仍应从更高阻值开始，再按`100 Ω -> 80 Ω -> 60 Ω -> 50 Ω`逐级检查。

切换约40 Ω重载：

```c
#define PFC_USER_ACTIVE_POWER_PROFILE         PFC_USER_POWER_HEAVY_40R
#define PFC_USER_ALLOW_UNPROTECTED_HEAVY_LOAD 1U
```

若选择`PFC_USER_POWER_HEAVY_40R`但强制宏仍为`0U`，编译器会直接报错，防止误下载。强制宏改为`1U`后只解除软件许可：

- OLED第4行显示`U:1`；
- VOFA CH9包含无保护重载标志；
- Keil Watch中`PFC_UnprotectedHeavyOverride=1`；
- 不会恢复IPFC、软件过流或周期级短路保护；
- F5A保险丝仍不能在单个PWM周期内保护MOSFET和电感。

重载档母线目标仍为60 V，虚拟电流上限约3.2 A RMS，参考斜率为0.75 V/s，并使用独立PI参数。不要为了更快升压跳过100 Ω、80 Ω、60 Ω和50 Ω阶段。
...
| ADC | Rank | 信号 | 当前用途 |
| --- | ---: | --- | --- |
| ADC1 | 1 | IPFC | 只记录原始码、一秒mean/min/max、VOFA CH8和Keil Watch |
| ADC1 | 2 | VBUS | 工程量换算、母线PI、AWD2过压与状态机 |
| ADC2 | 1 | VAC | 工程量换算、RMS、过零同步、AWD1与模型前馈 |

当前代码把`measurement.ipfc`、`ipfc_rms`、输入功率和软件PF明确置0。IPFC不参与：
...
因此`IPFC原始码看起来稳定`只证明ADC/DMA通道仍在工作，不能证明实际输入电流安全。
...
| 通道 | 数据 | 正常观察重点 |
| ---: | --- | --- |
| CH0 | VAC瞬时值 | 约50 Hz、正负对称 |
| CH1 | VBUS | 启动后逐步接近60 V |
| CH2 | VBUS参考 | 按档位斜率上升 |
| CH3 | 虚拟电流RMS指令`I*` | 非负且不长期顶住上限 |
| CH4 | 虚拟瞬时电流参考 | 与VAC同相的模型正弦量 |
| CH5 | 桥侧电压参考 | `VAC-L*di/dt` |
| CH6 | 调制度 | 应位于`-0.90~+0.90`且不持续削顶 |
| CH7 | 状态与故障组合码 | 状态应按启动流程推进 |
| CH8 | IPFC原始ADC码 | 只作通道监视，不换算电流 |
| CH9 | 强制重载标志与控制诊断码 | 轻载正常应为0 |

VOFA只有100 Hz刷新率，不能显示10 kHz开关细节，也不能作为过流仪表。真实输入电流必须用外部电流探头、功率分析仪或已校准的独立电流表测量。
...
- `M`是调制度乘10000。
- `S`是状态编号，`D`是控制失败细分码。
- `F`是锁存故障位，`U=1`表示无保护重载强制开启。

### 25.7 上电与停机操作

1. 保持`PFC_USER_RELAXED_PWM_TEST=0U`，确认选择100 Ω轻载档。
2. 接好示波器、电流探头和外部限流/断电手段；确认负载功率额定值足够。
3. 先上MCU和采样板电源，确认OLED无故障、VAC/VBUS显示合理。
4. 接入隔离36 V RMS交流，观察`VAC_RMS`、`vac_locked`和被动母线。
5. 等到`S=05`后短按PD0并释放。程序应先进入`S=11`，在新的正向过零后进入`S=06`。
6. 观察PA8~PA11驱动波形、CH0~CH9、外部输入电流和器件温升。
7. 需要正常停机时，在`S=06/07/08`再次短按PD0并释放；确认OLED进入`S=09`且四路PWM消失。
8. 紧急异常时不要依赖PD0消抖，直接断开功率输入。PD0是软件运行命令，不是硬件急停。

### 25.8 必须中止测试的现象

- 外部实测电流快速上升或超过当前电感、MOSFET、变压器和连线允许值；
- 调制度长时间贴近`+0.90`或`-0.90`；
- VBUS超过告警值或向66 V关断阈值快速上冲；
- 输入电流出现明显直流偏置、单边尖峰或桥臂异常声响；
- MOSFET、驱动器、电感、变压器、保险丝或母线电容快速升温；
- `F`或`D`非零、状态进入`S=10`、DMA/控制心跳停止；
- `U=1`但现场没有外部电流测量和快速断电手段。

PB10/HRTIM Fault 3、VAC/VBUS模拟看门狗、DMA同步、HRTIM写入和IWDG监督仍保留，但它们不能替代独立OCP/DESAT。没有可信电流采样和硬件短路保护时，不应把40 Ω档视为可安全长期运行的成品模式。
