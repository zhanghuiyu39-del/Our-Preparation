...
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : 36 V输入/60 V母线单相PWM整流器PI-only验证主程序
  ******************************************************************************
  * @attention
  *
 ...
 * 运行数据流：HRTIM TRG1 -> ADC/DMA回调 -> 测量快照/10 kHz PI+模型控制；主循环只处理
...
/*
 * VOFA JustFloat发送缓存，由主循环每10 ms写入并阻塞发送，固定使用10个通道。
...
static void PFC_DebugInitOledLabels(PFC_RunMode mode)
{
  ...
  else
  {
     ...
     */
    OLED_ShowString(1, 1, "VAC:    VB:     ");
    OLED_ShowString(2, 1, "BR:     I*:    ");
    OLED_ShowString(3, 1, "M:     S:  D:  ");
    OLED_ShowString(4, 1, "F:     U: ");
  }
}
                              ...
                              const PFC_ControlTelemetry *control)
{
  ...
  else if (pfc_params->run_mode == PFC_RUN_MODE_ENGINEERING_CHECK)
  {
    ...
    /* IPFC当前只保留原始码监视；禁止把不可信换算值发送成工程量。 */
    vofa_data[2] = (float)measurement->ipfc_raw;
    vofa_data[3] = 0.0f;
  ...
  {
    vofa_data[0] = measurement->vac;
    vofa_data[1] = measurement->vbus;
    vofa_data[2] = control->vbus_reference;
    vofa_data[3] = control->current_rms_command;
    vofa_data[4] = control->current_reference;
    vofa_data[5] = control->bridge_voltage_reference;
    vofa_data[6] = control->modulation;
    vofa_data[8] = (float)measurement->ipfc_raw;
    /* CH9高位为无保护重载标志，低两位为控制细分故障码。 */
    vofa_data[9] = (float)((uint32_t)control->unprotected_heavy_override * 100U +
                           (uint32_t)control->failure_reason);
  }
                                 ...
                                 const PFC_ControlTelemetry *control)
{
  ...
  {
    /*
     * 第1行：VAC为交流电压RMS乘10，VB为母线电压乘10，单位均为0.1 V。
     * IPFC不在OLED工程量页显示，避免把原始噪声误认为实测电流。
     */
    OLED_ShowNum(1, 5, (uint32_t)(measurement->vac_rms * 10.0f), 4);
    OLED_ShowNum(1, 12, (uint32_t)(measurement->vbus * 10.0f), 4);

    /*
     * 第2行：BR为母线参考乘10，单位0.1 V；I*为PI输出的虚拟电流RMS乘1000，
     * 仅表示模型指令，不是实测电流，也不承担过流保护。
     */
    OLED_ShowNum(2, 4, (uint32_t)(control->vbus_reference * 10.0f), 4);
    OLED_ShowNum(2, 12, (uint32_t)(control->current_rms_command * 1000.0f), 4);
     ...
     * 正常运行时D应为00；进入S=10且
      * F包含08000时，D能区分VBUS除法、PI数值、调制度和HRTIM写入等具体位置。
     */
    ...
    /*
     * 第4行：F为5位故障码；U为无保护重载强制标志。U=1只说明用户打开了
     * PFC_USER_ALLOW_UNPROTECTED_HEAVY_LOAD，绝不表示硬件OCP/DESAT已经存在。
     */
    OLED_ShowHexNum(4, 3, measurement->fault_bits, 5);
    OLED_ShowNum(4, 11, (uint32_t)control->unprotected_heavy_override, 1);
  }
}
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef *hadc)
{
  ...
  else if ((hadc != NULL) && (hadc->Instance == ADC1))
  {
    ...
    /* ADC1完整序列是唯一10 kHz控制入口；锁存故障后不得继续推进PI或模型调制。 */
