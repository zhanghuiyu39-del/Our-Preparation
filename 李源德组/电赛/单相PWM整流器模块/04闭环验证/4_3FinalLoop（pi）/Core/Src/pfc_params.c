...
volatile uint8_t PFC_UnprotectedHeavyOverride =
    (uint8_t)(((PFC_USER_ACTIVE_POWER_PROFILE == PFC_USER_POWER_HEAVY_40R) &&
               (PFC_USER_ALLOW_UNPROTECTED_HEAVY_LOAD != 0U)) ? 1U : 0U);
...
uint32_t PFC_Params_GetValidationError(const PFC_Params *params)
{
    ...
    if ((params->voltage_pi_kp < 0.0f) || (params->voltage_pi_ki < 0.0f) ||
    ...
    if ((params->vac_volt_per_count <= 0.0f) ||
        (params->vbus_volt_per_count <= 0.0f) ||
        (params->vac_zero_count > 4095U))
    {
        error |= PFC_PARAM_ERROR_CALIBRATION;
    }

    if (((params->vac_polarity != 1) && (params->vac_polarity != -1)) ||
