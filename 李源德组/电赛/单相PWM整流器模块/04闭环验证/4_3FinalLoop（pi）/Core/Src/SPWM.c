 ...
 * IPFC只用于监测和保护，因此当前实现不是电流闭环，也不能保证9 V精密稳压。
 */
#define SPWM_TWO_PI        6.28318530718f /* rad，一个完整电角周期。 */
#define SPWM_SQRT_TWO      1.41421356237f /* RMS到峰值的换算系数。 */
...
HAL_StatusTypeDef SPWM_ModelStep(const PFC_Measurement *measurement,
                                 float virtual_current_rms,
                                 float *virtual_current_instantaneous,
                                 float *inductor_voltage_reference,
                                 float *bridge_voltage_reference)
{
    ...
    if ((measurement == 0) || (SPWM_FloatValid(virtual_current_rms) == 0U) ||
        (virtual_current_rms < 0.0f) ||
        (SPWM_SyncUpdate(measurement) != HAL_OK) ||
        /* PRIME阶段虚拟电流为0，VAC/VBUS前馈与相位无关，可在首次过零前预装载。 */
        ((spwm_synchronized == 0U) && (virtual_current_rms > 0.0f)) ||
        (measurement->vbus <= SPWM_VBUS_DIV_MIN))
    {
        spwm_healthy = 0U;
        return HAL_ERROR;
    }

    current_peak = SPWM_SQRT_TWO * virtual_current_rms;
    omega = SPWM_TWO_PI * measurement->vac_frequency_hz;
    current_instantaneous = current_peak * sinf(spwm_phase);
    di_ref_dt = omega * current_peak * cosf(spwm_phase);
    inductor_voltage = spwm_params->input_inductance * di_ref_dt;
    bridge_voltage = measurement->vac - inductor_voltage;
    modulation = (float)spwm_params->bridge_polarity * bridge_voltage /
                 measurement->vbus;

    if ((SPWM_FloatValid(current_instantaneous) == 0U) ||
        (SPWM_FloatValid(inductor_voltage) == 0U) ||
        (SPWM_FloatValid(bridge_voltage) == 0U) ||
    ...
    if (virtual_current_instantaneous != 0)
    {
        *virtual_current_instantaneous = current_instantaneous;
    }
    if (inductor_voltage_reference != 0)
    {
        *inductor_voltage_reference = inductor_voltage;
    }
    if (bridge_voltage_reference != 0)
    {
        *bridge_voltage_reference = bridge_voltage;
    }
    return HAL_OK;
}
...
float SPWM_GetPhase(void) { return spwm_phase; }
