 ...
 *
 * Master/A/B计数器运行只代表ADC触发时基存在；WaveformOutputStart才会把PWM
...
/* 采样所需计数器集合：Master产生ADC触发，Timer A/B产生两桥臂PWM。 */
...
/**
 * @brief  启动Master、Timer A和Timer B，建立10 kHz采样与PWM时基。
 * @retval HAL_OK表示计数器已运行或此前已经启动；HAL_ERROR表示句柄无效或HAL启动失败。
 * @note   本函数不开放TA1/TA2/TB1/TB2，允许在ADC检查阶段只运行计数器。
 */
HAL_StatusTypeDef PFC_HRTIM_StartSampling(void)
{
    ...
    status = HAL_HRTIM_WaveformCounterStart(&hhrtim1, PFC_HRTIM_TIMERS);
    if (status == HAL_OK)
    {
 ...
 * @retval HAL_OK表示软件更新请求已写入，其他HAL状态表示句柄正忙或无效。
 * @note   在ADC1完整DMA ISR的PRIME投入路径调用；HAL实现只短暂加锁并写CR2，
 *         不等待PWM周期，也不会开放任何HRTIM输出。
 */
