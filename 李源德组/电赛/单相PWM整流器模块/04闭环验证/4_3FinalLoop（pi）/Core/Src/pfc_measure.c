...
/*
 * 本模块负责“ADC原始值 -> 同周期快照 -> VAC/VBUS工程量与工频统计”，不负责状态机和调制。
...
static uint8_t bipolar_rail_count = 0U;   /* VAC双极性前端贴轨连续帧数；IPFC不参与关断。 */
                                                 ...
                                                 ADC_HandleTypeDef *hadc2_handle)
{
    ...
    /* IPFC当前只作原始诊断，AWD1保持完整码域，不产生电流关断。 */
    ipfc_low = 0U;
    ipfc_high = 4095U;
...
void PFC_Measure_OnAdc1Complete(void)
{
    ...
    /*
     * 当前IPFC通道噪声和比例不可信，只保留ipfc_raw及一秒min/mean/max。
     * 下列工程量明确清零，避免OLED、VOFA或后续代码把换算噪声误认为实测电流、功率或PF。
     */
    pfc_measurement.ipfc = 0.0f;
    pfc_measurement.ipfc_rms = 0.0f;
    pfc_measurement.input_active_power_w = 0.0f;
    pfc_measurement.input_apparent_power_va = 0.0f;
    ...
    /* 当前没有可信IPFC反馈；该通道仅用于原始诊断，不能承担软件过流保护。 */
    if ((PFC_USER_RELAXED_PWM_TEST == 0U) &&
