 ...
 * 单相PWM整流器唯一人工配置入口。
 *
 * 使用顺序：先保持RAW_ADC记录原始统计值，再填写当前参数档的ADC零点、比例和
 * 极性；确认工程量正确后切换ENGINEERING_CHECK；最后才允许选择CLOSED_LOOP。
...
#define PFC_USER_PROFILE_36V_LIGHT_100R      1U /* 36 V RMS、60 V母线、100 ohm轻载闭环档；当前默认活动参数档。 */
#define PFC_USER_PROFILE_36V_CONTEST         2U /* 36 V RMS赛题满功率预留档；控制参数与轻载档分开保存。 */
#define PFC_USER_PROFILE_36V                 PFC_USER_PROFILE_36V_CONTEST /* 兼容旧名称；旧代码选择它时仍进入满功率赛题档。 */

/* 功率档只改变负载边界、PI上限和软启动参数，不改变ADC/HRTIM时基。 */
#define PFC_USER_POWER_LIGHT_100R             0U /* 36 V RMS、60 V母线、100 ohm轻载，默认调试档。 */
#define PFC_USER_POWER_HEAVY_40R              1U /* 36 V RMS、60 V母线、约40 ohm重载，必须逐级验证。 */
#define PFC_USER_ACTIVE_POWER_PROFILE         PFC_USER_POWER_LIGHT_100R

/* 保险丝不能提供微秒级短路保护；该宏只允许用户明确承担风险后手动打开重载档。 */
#define PFC_USER_ALLOW_UNPROTECTED_HEAVY_LOAD 0U
...
#define PFC_USER_MODE_CLOSED_LOOP            2U /* 带功率闭环模式；只能在工程量和保护阈值复核后选择。 */

/*
 * 日常只需要先改下面两行。推荐顺序固定为5V+RAW_ADC -> 5V+ENGINEERING_CHECK
 * -> 5V+CLOSED_LOOP。切换到36V不会自动重算ADC比例、PR/PI或保护阈值。
 */
#if PFC_USER_ACTIVE_POWER_PROFILE == PFC_USER_POWER_HEAVY_40R
#define PFC_USER_ACTIVE_PROFILE              PFC_USER_PROFILE_36V_CONTEST /* 重载档：约40 ohm/90 W，受无保护强制开关约束。 */
#else
#define PFC_USER_ACTIVE_PROFILE              PFC_USER_PROFILE_36V_LIGHT_100R /* 默认轻载：36 V RMS、60 V母线、100 ohm。 */
#endif
#define PFC_USER_RUN_MODE                    PFC_USER_MODE_CLOSED_LOOP /* 默认允许状态机在实时输入条件满足后，通过PD0短按投入PI-only母线闭环。 */

/*
 * 闭环PWM波形验证开关：
 * 1U = 当前调试用法。只要参数、ADC标定和同步工程量有效，状态机即可进入READY；
 *      不再用VAC幅值/频率、母线启动门槛、电流跟踪、目标建立超时、ADC看门狗、
 *      软件过压、ADC贴轨、PI/SPWM限幅和母线稳压失调来关闭PWM。上述诊断量
 *      仍继续统计，可在OLED、VOFA或Keil Watch中观察控制是否正确。
 * 0U = 恢复完整状态机判据和软件保护，准备正式带载前应切回0U并逐项验证阈值。
 *
 * 修改后果：设置为1U不会改变PI、PWM频率、死区、调制度上限或虚拟电流指令，
...
#define PFC_USER_5V_IPFC_A_PER_COUNT         0.003322f     /* 备用A/count；当前只显示IPFC原始码，本比例不进入控制、保护或工程量发布。 */
...
#define PFC_USER_5V_CALIBRATION_CONFIRMED    1U         /* 0=只允许RAW_ADC；全部零点/比例/极性/阈值实测后才改1，改1本身不会自动完成标定。 */
...
#define PFC_USER_5V_CURRENT_RAMP_A_PER_S     0.50f      /* 备用A/s，旧开环SPWM使用；PI-only POWER_RAMP实际使用PROBE_SLEW。 */

/*
 * 1 kHz PI、虚拟电流模型、软启动和超时参数。当前没有可信IPFC反馈，PR参数只作为
 * 将来恢复电流环的备用值，不进入运行路径。PI一次只改一个增益，并保存VAC、VBUS、
 * VBUS参考、I*和调制度波形；增益调大通常响应更快，但超调和低频振荡风险也会增加。
 */
...
#define PFC_USER_5V_CURRENT_ERROR_MAX_A_RMS  0.10f      /* 备用A RMS；当前无可信电流反馈，不参与状态迁移或关断。 */
#define PFC_USER_5V_PROBE_MIN_MS             60U        /* ms，PR投入后最短观察时间；至少覆盖一个50 Hz周期20 ms并留出斜坡时间，过短不能可靠判断。 */
#define PFC_USER_5V_CURRENT_TIMEOUT_MS       500U       /* 备用ms；当前PI-only状态机不执行电流环建立超时。 */
...
#define PFC_USER_36V_LOAD_RESISTANCE_OHM     40.0f      /* ohm，60 V时约90 W；无电流反馈时只能在显式强制开关打开后使用。 */
...
#define PFC_USER_36V_IPFC_A_PER_COUNT        0.003323f  /* 备用A/count；当前只发布IPFC原始码，不进行电流换算或软件过流。 */
 ...
 * PI方向、母线稳压和调制度趋势，不代表赛题2 A输出、95%效率或THD已通过。负载在该工作点耗散
...
#define PFC_USER_36V_LIGHT_CURRENT_TRIP_A_PEAK     4.00f      /* 备用A peak；当前IPFC不可信，本值不执行软件关断，也不收紧ADC1 AWD1。 */
...
#define PFC_USER_36V_LIGHT_CURRENT_RAMP_A_PER_S    1.00f      /* 备用A/s，旧开环接口使用；PI-only POWER_RAMP实际使用PROBE_SLEW。 */

#define PFC_USER_36V_LIGHT_PR_KP                   0.50f      /* 备用V/A，当前PI-only路径不读取，留作将来恢复PR电流环。 */
...
#define PFC_USER_36V_LIGHT_PROBE_CURRENT_A_RMS     0.20f      /* A RMS，POWER_RAMP的虚拟电流预置目标；不是实测电流。 */
#define PFC_USER_36V_LIGHT_PROBE_SLEW_A_PER_S      2.0f       /* A RMS/s，到0.20 A约100 ms；调快会增大首批能量注入和母线过冲。 */
#define PFC_USER_36V_LIGHT_CURRENT_ERROR_MAX_A_RMS 0.18f      /* 备用A RMS，当前无IPFC反馈时不参与任何状态迁移或关断。 */
#define PFC_USER_36V_LIGHT_PROBE_MIN_MS            250U       /* ms，POWER_RAMP保持时间；到时无扰投入母线PI，不执行电流资格判断。 */
#define PFC_USER_36V_LIGHT_CURRENT_TIMEOUT_MS      2500U      /* 备用ms，当前PI-only状态机不执行电流环建立超时。 */
...
/* ======================== 5. 36 V赛题满功率参数档 ======================== */
/* 下列原有参数继续作为约111 W赛题档骨架保存，禁止直接用于首次36 V轻载上电。 */
...
#define PFC_USER_36V_CURRENT_TARGET_A_PEAK   4.525f     /* A peak，对应虚拟电流上限约3.20 A RMS；不是实测限流值。 */
...
#define PFC_USER_36V_VBUS_SLEW_V_PER_S       0.75f      /* V/s，无保护重载档采用更慢参考斜坡；仍不能替代电流保护。 */
#define PFC_USER_36V_PROBE_CURRENT_A_RMS     0.50f      /* A RMS，重载POWER_RAMP虚拟电流预置目标，不是实测电流。 */
...
#define PFC_USER_36V_CURRENT_TIMEOUT_MS      500U       /* 备用ms，当前PI-only状态机不执行电流环资格超时。 */
...
/* ======================== 7. 编译期安全检查 ======================== */
#if ((PFC_USER_ACTIVE_POWER_PROFILE != PFC_USER_POWER_LIGHT_100R) && \
     (PFC_USER_ACTIVE_POWER_PROFILE != PFC_USER_POWER_HEAVY_40R))
#error "PFC_USER_ACTIVE_POWER_PROFILE is invalid"
#endif

#if ((PFC_USER_ALLOW_UNPROTECTED_HEAVY_LOAD != 0U) && \
     (PFC_USER_ALLOW_UNPROTECTED_HEAVY_LOAD != 1U))
#error "PFC_USER_ALLOW_UNPROTECTED_HEAVY_LOAD must be 0U or 1U"
#endif

#if (PFC_USER_ACTIVE_POWER_PROFILE == PFC_USER_POWER_HEAVY_40R) && \
    (PFC_USER_ALLOW_UNPROTECTED_HEAVY_LOAD == 0U)
#error "Heavy load requires explicit PFC_USER_ALLOW_UNPROTECTED_HEAVY_LOAD=1U"
#endif

/* 重载强制只解除“缺少电流保护”许可，禁止同时关闭VAC/VBUS和限幅保护。 */
#if (PFC_USER_ACTIVE_POWER_PROFILE == PFC_USER_POWER_HEAVY_40R) && \
    (PFC_USER_RELAXED_PWM_TEST != 0U)
#error "Heavy load requires PFC_USER_RELAXED_PWM_TEST=0U"
#endif
...
#if ((PFC_USER_5V_VAC_ZERO_COUNT > 4095U) || \
     (PFC_USER_36V_VAC_ZERO_COUNT > 4095U))
#error "VAC zero count must be in the 12-bit range"
#endif

#if (((PFC_USER_5V_VAC_POLARITY != 1) && (PFC_USER_5V_VAC_POLARITY != -1)) || \
...
#error "VAC and bridge polarity must be +1 or -1"
#endif
