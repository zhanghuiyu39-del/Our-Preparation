...
/* PI-only母线闭环公共契约：10 kHz ISR写控制状态，主循环只读取遥测快照。 */
...
/** @brief PI-only控制器的运行阶段，由PFC应用状态机显式切换。 */
typedef enum
{
    PFC_CONTROL_IDLE = 0,       /* 控制器已复位，不写HRTIM Compare。 */
    PFC_CONTROL_POWER_RAMP = 1,   /* 模型虚拟电流RMS指令由0按斜率增加。 */
    PFC_CONTROL_VBUS_RAMP = 2,    /* PI外环已投入，VBUS参考斜坡上升。 */
    PFC_CONTROL_VBUS_RUN = 3,     /* 参数档母线目标附近的稳态PI-only闭环。 */
    PFC_CONTROL_PRIME = 4         /* 输出关闭时跟踪VAC/VBUS前馈，等待正向过零投入。 */
} PFC_ControlMode;

/* 兼容旧Watch表达式；新代码和文档统一使用PFC_CONTROL_POWER_RAMP。 */
#define PFC_CONTROL_CURRENT_RAMP PFC_CONTROL_POWER_RAMP

/**
 * @brief  PI-only控制器每个10 kHz周期使用的最小输入快照。
 * @note   输入由ADC1完整DMA回调从一致性测量快照转换而来；IPFC故意不在此结构体中出现，
 *         因为当前没有可信电流反馈，不能把它误接回控制环。
 */
typedef struct
{
    float vac_v;                         /* V，已按VAC零点/比例换算的瞬时交流电压。 */
    float vbus_v;                        /* V，已按VBUS比例换算的瞬时母线电压。 */
    float vac_frequency_hz;              /* Hz，过零测得的输入频率。 */
    uint32_t vac_zero_cross_sequence;    /* 每次VAC负到正过零加1，用于相位重新对齐。 */
    uint8_t vac_locked;                  /* 1=频率与周期在允许窗口内，0=暂不能投入能量。 */
} PFC_ControlInput;

/**
 * @brief  PI-only控制器一次计算发布的工程量。
 * @note   virtual_current_* 是模型指令，不是实测电流；主循环和VOFA可读取，不能当作过流保护。
 */
typedef struct
{
    float vbus_reference;                 /* V，母线PI当前参考值。 */
    float virtual_current_rms;            /* A RMS，PI输出的虚拟输入电流需求。 */
    float virtual_current_instantaneous;  /* A，按VAC相位生成的模型瞬时电流参考。 */
    float inductor_voltage_reference;     /* V，L*di/dt前馈电压。 */
    float bridge_voltage_reference;       /* V，VAC-vL得到的桥侧电压参考。 */
    float modulation;                     /* 归一化调制度，已经过软件限幅。 */
    uint8_t pi_limited;                   /* 1=PI输出触及上下限。 */
    uint8_t modulation_limited;           /* 1=SPWM调制度触及上下限。 */
    uint32_t fault_bits;                  /* PFC_FaultMask组合；非零时必须停PWM。 */
} PFC_ControlOutput;
...
typedef enum
{
    ...
    PFC_CONTROL_FAIL_INIT_CONTROLLER = 0x02,   /* PI、陷波器系数或SPWM初始化失败。 */
    PFC_CONTROL_FAIL_NOT_INITIALIZED = 0x03,   /* 10 kHz路径开始时控制模块或参数尚未初始化。 */
    PFC_CONTROL_FAIL_MEASUREMENT = 0x04,       /* 测量指针为空或一致性快照valid为0。 */
    PFC_CONTROL_FAIL_CONTROLLER_STATE = 0x05,  /* PI内部initialized标志异常。 */
    ...
    PFC_CONTROL_FAIL_CURRENT_ERROR = 0x0B,     /* 保留诊断码：当前没有实测电流误差。 */
    PFC_CONTROL_FAIL_PR_RAW = 0x0C,            /* 保留诊断码：当前未调用PR。 */
    PFC_CONTROL_FAIL_PR_OUTPUT = 0x0D,         /* 保留诊断码：当前未调用PR。 */
...
typedef struct
{
    ...
    float ipfc_a;                     /* A，当前固定为0；IPFC只在测量模块保留原始码诊断。 */
    float current_rms_command_a;      /* A RMS，PI输出的虚拟电流指令，不是实测限流值。 */
    float current_reference_a;        /* A，模型生成的瞬时电流参考，不是反馈电流。 */
    float current_error_a;            /* A，PI-only模式固定为0，保留字段兼容旧Watch布局。 */
    float pr_raw_output_v;            /* V，保留字段；当前未调用PR，固定为0。 */
    float pr_output_v;                /* V，实际保存L*di/dt前馈，字段名为旧Watch兼容。 */
    float bridge_reference_v;         /* V，VAC-L*di/dt得到的桥侧电压参考。 */
...
typedef struct
{
    ...
    float current_rms_command;  /* A RMS，PI输出的虚拟模型电流指令，不是实测电流。 */
    float current_reference;    /* A，模型Ipk*sin(theta)，不参与实测电流闭环。 */
    float current_error;        /* 当前PI-only模式固定为0；IPFC不参与误差计算。 */
    float current_error_rms;    /* 当前PI-only模式固定为0。 */
    float pr_output_v;          /* 当前保存L*di/dt前馈电压，字段名仅为旧调试兼容。 */
    float inductor_voltage_reference; /* V，模型L*di/dt前馈电压。 */
    float bridge_voltage_reference;   /* V，VAC-L*di/dt桥侧电压参考。 */
    float phase_rad;                  /* rad，VAC正向过零同步相位。 */
...
/**
 * @brief  初始化母线PI、100 Hz陷波器、SPWM底层和遥测状态。
...
/**
 * @brief  切换PI-only阶段，并完成进入该阶段所需的状态预置。
 * @param  mode 目标控制模式。
...
/**
 * @brief  执行一次10 kHz虚拟电流模型，并按1/10分频执行陷波器和母线PI。
 * @param  input ADC1完整DMA回调刚发布的控制输入快照。
 * @param  output 可选输出结构；非空时复制本次计算结果。
 ...
 * @param  input 开放输出时使用的PI-only控制输入快照。
...
/**
 * @brief  清除PI、陷波器、分频、虚拟电流斜坡和遥测运行状态。
