...
/**
 * @brief  根据PI给出的虚拟电流RMS指令执行一次VAC同步电感模型调制。
 * @param  measurement 当前10 kHz一致性电压快照；IPFC字段不会被读取。
 * @param  virtual_current_rms A RMS，母线PI要求的模型电流幅值，必须为非负有限数。
 * @param  virtual_current_instantaneous 可选输出，返回Ipk*sin(theta)，单位A。
 * @param  inductor_voltage_reference 可选输出，返回L*di/dt，单位V。
 * @param  bridge_voltage_reference 可选输出，返回VAC-L*di/dt，单位V。
 * @retval HAL_OK表示相位、模型和Compare写入均有效，否则返回HAL_ERROR。
 * @note   仅允许在ADC1完整DMA ISR中调用；该模型没有电流反馈，不能提供过流保护。
 */
...
/** @brief 返回VAC同步相位，单位rad，范围0到2*pi。 */
float SPWM_GetPhase(void);
