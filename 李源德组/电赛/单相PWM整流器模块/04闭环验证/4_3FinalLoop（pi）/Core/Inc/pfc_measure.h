...
typedef enum
{
    ...
    PFC_FAULT_OVERCURRENT      = 1U << 4,  /* 预留：仅恢复可信IPFC后才允许使用。 */
    ...
    PFC_FAULT_AWD_IPFC         = 1U << 12, /* 预留：当前IPFC AWD1宽窗口且不关断。 */
    ...
    PFC_FAULT_CONTROL_SATURATION = 1U << 17,/* PR或调制度连续限幅，或周期削顶占比连续超限。 */
    PFC_FAULT_VBUS_REGULATION  = 1U << 18  /* PI已限幅但母线持续无法保持目标。 */
} PFC_FaultMask;
...
typedef struct
{
    ...
    float ipfc;             /* A，当前固定为0；IPFC只发布原始码，不参与PI-only控制。 */
    float ipfc_rms;         /* A，当前固定为0；没有可信电流反馈时禁止当作保护量。 */
    float vac;              /* V，瞬时交流输入电压。 */
    float vac_rms;          /* V，一个标称工频周期更新一次。 */
    float vac_frequency_hz; /* Hz，由相邻VAC正向过零计算。 */
    float vbus;             /* V，直流母线电压。 */
    float input_active_power_w;  /* W，当前固定为0；没有可信IPFC时不计算输入功率。 */
    float input_apparent_power_va; /* VA，当前固定为0；不由噪声换算结果伪造视在功率。 */
    float input_power_factor;    /* -1~+1，P/S软件估算值；正值表示当前极性定义下吸收有功功率。 */
    ...
    uint8_t input_power_valid; /* 1表示已完成一整个工频窗口且视在功率足以可靠计算PF。 */
    uint8_t valid;          /* VAC/VBUS工程量和同步有效；不代表IPFC工程量有效。 */
} PFC_Measurement;
...
/**
 * @brief  在短临界区内复制ISR发布的完整测量快照。
 * @param  measurement 接收快照的非空地址。
 * @note   主循环和控制模块不得绕过该接口直接读取DMA数组；临界区不冻结DMA硬件。
 */
