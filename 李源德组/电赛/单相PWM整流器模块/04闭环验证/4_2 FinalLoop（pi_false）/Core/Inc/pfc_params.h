...
typedef enum
{
    PFC_PROFILE_5V_TEST = 0,  /* 5 V RMS、30 ohm负载的隔离低压虚拟反馈试验。 */
...
typedef enum
{
    ...
    PFC_RUN_MODE_VIRTUAL_FEEDBACK = PFC_USER_MODE_VIRTUAL_FEEDBACK,
    PFC_RUN_MODE_CLOSED_LOOP = PFC_RUN_MODE_VIRTUAL_FEEDBACK /* 兼容旧调用点。 */
} PFC_RunMode;
 ...
 * ADC比例、零点和极性必须来自实际板卡标定；标定未确认或非带功率模式时，软件只能
...
typedef struct
{
    ...
    uint8_t adc_enabled;              /* 1启动ADC/DMA/AWD，0改用HRTIM独立10 kHz节拍。 */
    uint8_t pwm_output_enabled;       /* 1允许状态机响应PD0开放PWM，0强制只运行后台。 */
    ...
    float virtual_vbus_feedback;       /* V，固定送入PI的虚拟母线反馈，不是ADC实测VBUS。 */
    float virtual_current_preset_rms;  /* A RMS，进入虚拟模式时PI的预置输出。 */
    float virtual_current_slew_a_s;    /* A RMS/s，预置电流送入开环前馈的斜率。 */
} PFC_Params;
 ...
 * @param  params 待检查的常驻参数地址。
 * @retval 1仅表示虚拟反馈功率模式、标定确认和参数检查全部通过。
