...
/* PFC状态机公共接口：连接1 ms主循环、独立10 kHz虚拟控制和HRTIM功率许可。 */

#include <stdint.h>
#include "pfc_measure.h"
#include "pfc_params.h"

/** @brief 单相PWM整流器从安全采样到虚拟反馈开环功率运行的应用状态。 */
typedef enum
{
    ...
    PFC_VIRTUAL_RAMP,      /* PWM已开放，0.10 A RMS虚拟PI输出按斜率投入。 */
    PFC_VIRTUAL_RUN,       /* 固定虚拟反馈PI与开环电感前馈持续运行。 */
    PFC_STOP,              /* 人工停机后的短暂等待，采样计数器仍运行。 */
    PFC_FAULT_LATCH        /* 故障锁存，软件不允许自动恢复。 */
} PFC_State;

/**
 * @brief  初始化PFC状态机、PD0消抖和虚拟反馈PI控制器。
...
/**
 * @brief  ADC1 DMA或HRTIM Master回调中的唯一10 kHz控制入口。
 * @note   ADC关闭时由HRTIM ISR调用；不允许OLED、USART、HAL_Delay()或阻塞操作。
 */
...
/**
 * @brief  检查当前活动快速时基、控制心跳、状态机和HRTIM状态是否一致。
