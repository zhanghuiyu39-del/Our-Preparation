 ...
 *
 * 当前启动数据流：GPIO安全态 -> HRTIM独立10 kHz时基 -> IWDG；ADC/DMA/AWD均跳过。
 * 运行数据流：HRTIM Master Repetition -> 虚拟PI/软件相位/SPWM；主循环只处理
 * 1 ms状态机、10 ms VOFA、500 ms OLED和100 ms安全监督。
 *
 * 当前板卡没有MCU可控Gate Enable。打开TA1/TA2/TB1/TB2即可能驱动功率器件；
 * 当前无ADC模式不发布实测量；外设启用与PWM许可由pfc_user_config.h分层控制。
 */
...
static void PFC_DebugInitOledLabels(PFC_RunMode mode)
{
  if ((pfc_params != NULL) && (pfc_params->adc_enabled == 0U))
  {
    OLED_ShowString(1, 1, "R:     X:     ");
    OLED_ShowString(2, 1, "A:     M:     ");
    OLED_ShowString(3, 1, "H:     O: ");
    OLED_ShowString(4, 1, "F:     S:  ");
  }
  else if (mode == PFC_RUN_MODE_RAW_ADC)
  {
  ...
  else
  {
    ...
    OLED_ShowString(2, 1, "D:     X:     ");
    OLED_ShowString(3, 1, "A:     M:     ");
    OLED_ShowString(4, 1, "F:     S:  ");
  }
}
                              ...
                              const PFC_ControlTelemetry *control)
{
  ...
  if (pfc_params->adc_enabled == 0U)
  {
    vofa_data[0] = control->synthetic_vac_reference;
    vofa_data[1] = control->current_reference;
    vofa_data[2] = control->virtual_vbus_reference;
    vofa_data[3] = control->virtual_vbus_feedback;
    vofa_data[4] = control->pi_output_current_rms;
    vofa_data[5] = control->applied_current_rms;
    vofa_data[6] = control->modulation;
  }
  else if (pfc_params->run_mode == PFC_RUN_MODE_RAW_ADC)
  {
  ...
  else
  {
    ...
    vofa_data[4] = control->virtual_vbus_feedback;
    vofa_data[5] = control->applied_current_rms;
    vofa_data[6] = control->modulation;
  }
 ...
 * @note  主循环按用户配置周期调用；ADC关闭时显示虚拟量，禁止从HRTIM ISR调用。
 */
static void PFC_DebugRefreshOled(const PFC_Measurement *measurement,
                                 const PFC_AdcCalibrationStats *stats,
                                 const PFC_ControlTelemetry *control)
{
  ...
  if (pfc_params->adc_enabled == 0U)
  {
    OLED_ShowNum(1, 3, (uint32_t)(control->virtual_vbus_reference * 100.0f), 5);
    OLED_ShowNum(1, 11, (uint32_t)(control->virtual_vbus_feedback * 100.0f), 5);
    OLED_ShowNum(2, 3, (uint32_t)(control->applied_current_rms * 1000.0f), 5);
    OLED_ShowSignedNum(2, 11, (int32_t)(control->modulation * 10000.0f), 5);
    OLED_ShowNum(3, 3, control->fast_heartbeat % 100000U, 5);
    OLED_ShowNum(3, 11, PFC_HRTIM_OutputsEnabled(), 1);
    OLED_ShowHexNum(4, 3, measurement->fault_bits, 5);
    OLED_ShowNum(4, 11, (uint32_t)PFC_AppGetState(), 2);
  }
  else if (pfc_params->run_mode == PFC_RUN_MODE_RAW_ADC)
  {
  ...
  else
  {
    ...
    OLED_ShowNum(2, 11, (uint32_t)(control->virtual_vbus_feedback * 100.0f), 5);
    OLED_ShowNum(3, 3, (uint32_t)(control->applied_current_rms * 1000.0f), 5);
    OLED_ShowSignedNum(3, 11, (int32_t)(control->modulation * 10000.0f), 5);
    OLED_ShowHexNum(4, 3, measurement->fault_bits, 5);
    OLED_ShowNum(4, 11, (uint32_t)PFC_AppGetState(), 2);
  }
}
...
int main(void)
{
  ...
  /*
   * 启动Master、Timer A/B计数器和Master Repetition中断。
   * ADC关闭时由该10 kHz中断驱动虚拟PI、软件相位和SPWM更新；仍不开放PWM输出。
   */
  ...
  /*
   * IWDG放在可能耗时或失败的初始化之后启动。无ADC模式只在HRTIM快速时基、
   * 控制心跳、状态机和输出状态一致时刷新，避免ADC停用造成约500 ms循环复位。
   */
  MX_IWDG_Init();
  ...
  /* ms Tick；主循环独占写入，OLED周期由用户配置统一决定。 */
  ...
  while (1)
  {
	 ...
	 */
	if ((HAL_GetTick() - oled_tick) >= PFC_USER_OLED_PERIOD_MS)
    {
...
#else
      /* 无ADC模式只监督HRTIM 10 kHz时基、控制心跳、Fault和输出状态。 */
      if (PFC_AppWatchdogHealthy() != 0U)
      {
        (void)HAL_IWDG_Refresh(&hiwdg);
      }
#endif
...
/**
 * @brief HRTIM Master每个10 kHz周期结束时的独立快速控制入口。
 * @param hhrtim 预期为HRTIM1句柄。
 * @param TimerIdx 预期为HRTIM_TIMERINDEX_MASTER。
 * @note ADC关闭时替代ADC1 DMA回调；ISR内禁止OLED、USART、HAL_Delay和阻塞操作。
 */
#if !PFC_USER_ENABLE_ADC
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef *hadc)
{
  ...
  else if ((hadc != NULL) && (hadc->Instance == ADC1))
  {
    ...
    /* ADC1完整序列是唯一10 kHz控制入口；锁存故障后不得继续推进虚拟PI或调制。 */
