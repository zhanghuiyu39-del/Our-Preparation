...
/*
 * 本模块负责PD0人工启停、虚拟反馈阶段迁移和功率输出许可，不负责ADC换算或PI计算。
 * 数据流：1 ms主循环 -> 按键消抖/状态机 -> HRTIM输出启停；
 *           ADC1 DMA或HRTIM Master Repetition ISR -> PFC_AppFastStep()
 *           -> 虚拟PI/开环前馈和Compare更新。
 *
...
/* ADC关闭时的独立10 kHz快速入口心跳；由HRTIM ISR写，主循环每100 ms读取。 */
static volatile uint32_t app_timebase_heartbeat = 0U;
static uint32_t app_last_timebase_heartbeat = 0U;
...
static uint8_t PFC_AppInputReady(const PFC_Measurement *measurement)
{
    ...
    if (app_params->adc_enabled == 0U)
    {
        /* 无ADC模式没有VAC/VBUS反馈和被动预充判据，只检查参数与Fault锁存。 */
        return (uint8_t)(PFC_Measure_GetFault() == PFC_FAULT_NONE);
    }
...
/**
 * @brief  复核许可条件，预置虚拟PI、写中性Compare并开放四路HRTIM输出。
...
static void PFC_AppStartPower(const PFC_Measurement *measurement)
{
    ...
    if ((PFC_Control_EnterMode(PFC_CONTROL_VIRTUAL_RAMP, measurement) != HAL_OK) ||
    ...
    app_state = PFC_VIRTUAL_RAMP;
    app_state_time_ms = 0U;
}

/**
 * @brief  初始化PFC状态、PD0消抖、IWDG复位记录和虚拟反馈PI模块。
...
void PFC_AppInit(const PFC_Params *params, uint8_t iwdg_reset_seen)
{
    ...
    app_timebase_heartbeat = 0U;
    app_last_timebase_heartbeat = 0U;
    key_raw_last = 0U;
    key_stable = 0U;
...
/**
 * @brief  完成一次10 kHz开环前馈及必要的1 kHz虚拟PI。
 * @note   由ADC1 DMA或HRTIM Master回调以10 kHz调用；禁止阻塞、OLED、USART和HAL_Delay()。
 */
void PFC_AppFastStep(void)
{
    PFC_Measurement measurement;

    app_timebase_heartbeat++;
    (void)memset((void *)&measurement, 0, sizeof(measurement));
    measurement.valid = 1U;
    measurement.raw_valid = 1U;
    measurement.vac_locked = (app_params != 0) &&
                             (app_params->adc_enabled == 0U);
    measurement.vac_frequency_hz = (app_params != 0) ?
                                   app_params->grid_frequency_hz : 50.0f;
    measurement.fault_bits = PFC_Measure_GetFault();

    if (app_params != 0 && app_params->adc_enabled != 0U)
    {
    ...
    if ((app_state == PFC_VIRTUAL_RAMP) ||
        (app_state == PFC_VIRTUAL_RUN))
    {
...
void PFC_AppTick1ms(void)
{
    ...
    case PFC_SAFE:
        if (app_state_time_ms >= ((app_iwdg_reset_seen != 0U) ? 1000U : 100U))
        {
            ...
            {
                app_state = (app_params->adc_enabled == 0U) ?
                             PFC_READY : PFC_ADC_CHECK;
            }
            else
            {
    ...
    case PFC_VIRTUAL_RAMP:
        ...
        else if ((app_params->adc_enabled != 0U) &&
                 ((measurement.vac_locked == 0U) ||
        ...
        else
        {
            PFC_ControlTelemetry telemetry;
            PFC_Control_GetTelemetry(&telemetry);
            if (telemetry.ramp_complete != 0U)
            {
                if (PFC_Control_EnterMode(PFC_CONTROL_VIRTUAL_RUN,
                                          &measurement) != HAL_OK)
                {
            ...
            {
                /* 软启动仍未达到PI输出说明参数或控制节拍异常，不允许无限带功率等待。 */
                PFC_AppTrip(PFC_FAULT_TARGET_TIMEOUT);
            }
        }
        break;

    case PFC_VIRTUAL_RUN:
        if (short_press != 0U)
        {
            PFC_AppStopPower();
            app_state = PFC_STOP;
    ...
    case PFC_STOP:
        if (app_state_time_ms >= 100U)
        {
            ...
            else if (PFC_AppInputReady(&measurement) != 0U)
            {
                PFC_AppEnterReady();
            }
            else
...
/**
 * @brief  检查最近监督窗口内的活动时基、控制心跳、状态机和HRTIM状态一致性。
...
uint8_t PFC_AppWatchdogHealthy(void)
{
    ...
    power_state = (uint8_t)((app_state == PFC_VIRTUAL_RAMP) ||
                            (app_state == PFC_VIRTUAL_RUN));
    if ((app_params != 0) && (app_params->adc_enabled == 0U))
    {
        healthy = (uint8_t)((app_timebase_heartbeat != app_last_timebase_heartbeat) &&
        ...
        app_last_timebase_heartbeat = app_timebase_heartbeat;
