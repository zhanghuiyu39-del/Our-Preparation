...
#define PFC_CFG_VIRTUAL_VBUS_FEEDBACK      PFC_USER_5V_VIRTUAL_VBUS_FEEDBACK_V
#define PFC_CFG_VIRTUAL_CURRENT_PRESET     PFC_USER_5V_VIRTUAL_CURRENT_PRESET_A_RMS
#define PFC_CFG_VIRTUAL_CURRENT_SLEW       PFC_USER_5V_VIRTUAL_CURRENT_SLEW_A_S
#else
...
#define PFC_CFG_VIRTUAL_VBUS_FEEDBACK      PFC_USER_36V_VIRTUAL_VBUS_FEEDBACK_V
#define PFC_CFG_VIRTUAL_CURRENT_PRESET     PFC_USER_36V_VIRTUAL_CURRENT_PRESET_A_RMS
#define PFC_CFG_VIRTUAL_CURRENT_SLEW       PFC_USER_36V_VIRTUAL_CURRENT_SLEW_A_S
#endif
...
static const PFC_Params pfc_active_params =
{
    ...
    PFC_CFG_VIRTUAL_VBUS_FEEDBACK, PFC_CFG_VIRTUAL_CURRENT_PRESET,
    PFC_CFG_VIRTUAL_CURRENT_SLEW
};
...
uint32_t PFC_Params_GetValidationError(const PFC_Params *params)
{
    ...
    if (((params->profile_id != PFC_PROFILE_5V_TEST) &&
        ...
        ((params->run_mode != PFC_RUN_MODE_RAW_ADC) &&
         (params->run_mode != PFC_RUN_MODE_ENGINEERING_CHECK) &&
          (params->run_mode != PFC_RUN_MODE_VIRTUAL_FEEDBACK)))
    {
        error |= PFC_PARAM_ERROR_MODE;
    }
    if ((params->adc_enabled > 1U) || (params->pwm_output_enabled > 1U) ||
        ((params->run_mode != PFC_RUN_MODE_VIRTUAL_FEEDBACK) &&
         (params->pwm_output_enabled != 0U)))
    {
        error |= PFC_PARAM_ERROR_MODE;
    }
    ...
    if ((params->virtual_vbus_feedback <= 0.0f) ||
        (params->virtual_current_preset_rms <= 0.0f) ||
        (params->virtual_current_slew_a_s <= 0.0f) ||
        (params->virtual_current_preset_rms >=
         (params->current_command_target / 1.41421356237f)))
    {
        error |= PFC_PARAM_ERROR_CONTROL;
    }
...
{
    if ((params == 0) || (PFC_Params_Validate(params) == 0U) ||
        (params->run_mode != PFC_RUN_MODE_VIRTUAL_FEEDBACK) ||
        (params->pwm_output_enabled == 0U))
    {
        return 0U;
    }
    return (uint8_t)((params->adc_enabled == 0U) ||
                     (params->calibration_confirmed != 0U));
}
